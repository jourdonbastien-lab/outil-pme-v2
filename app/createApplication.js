...
function createApplication(options = {}) {
  ...
  const PROJECT_ROOT = options.projectRoot || require('path').resolve(__dirname, '..');
  ...
  const { configureTrustProxy, configureMiddleware } = require('./configureMiddleware');
  const { registerProcessErrorHandlers, registerExpressErrorHandler } = require('./errorHandlers');
  ...
  configureTrustProxy(app, TRUST_PROXY);
  ...
  function resolveStoragePath(value, fallback) {
    ...
    const target = raw || path.join(PROJECT_ROOT, 'storage');
    return path.isAbsolute(target) ? path.normalize(target) : path.join(PROJECT_ROOT, target);
  }
  
  const STORAGE_DIR = resolveStoragePath(process.env.OUTIL_PME_STORAGE_DIR, path.join(PROJECT_ROOT, 'storage'));
  ...
  const MEASUREMENTS_PUBLIC_DIR = path.join(PROJECT_ROOT, 'modules', 'measurements', 'public');
  ...
  configureMiddleware(app, {
    express, path, projectRoot: PROJECT_ROOT, session, sessionSecret: SESSION_SECRET,
    sessionStore, trustProxy: TRUST_PROXY, sessionMaxAgeMs: SESSION_MAX_AGE_MS,
    sessionCookieSecure: SESSION_COOKIE_SECURE, sessionCookieSameSite: SESSION_COOKIE_SAMESITE
  });
  ...
  const authController = createAuthController({
    ...
    authEmailService,
    twoFactorView,
    loginFilePath: path.join(PROJECT_ROOT, 'public', 'login.html'),
  ...
  registerProcessErrorHandlers(process, console);
  registerExpressErrorHandler(app, console);
  /* ===================== TÂCHES ===================== */
  ...
  return {
    app, db, sessionStore, agendaService, incomingDocumentsImportService,
    scannerImportEnabled: SCANNER_IMPORT_ENABLED, host: HOST, port: PORT
  };
}
