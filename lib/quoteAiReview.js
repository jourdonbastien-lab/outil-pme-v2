...
async function requestOpenAiInterpretation({
  apiKey, model, safePayload, deterministicRisk, fetchImpl = globalThis.fetch,
  AbortControllerImpl, setTimeoutImpl = setTimeout, clearTimeoutImpl = clearTimeout, timeoutMs
}) {
  ...
  const controller = AbortControllerImpl && Number(timeoutMs) > 0 ? new AbortControllerImpl() : null;
  const timeout = controller ? setTimeoutImpl(() => controller.abort(), Number(timeoutMs)) : null;
  try {
