...
async function listGoogleCalendarEvents(calendar, calendarId, options = {}) {
  const fetchAllPages = async (syncToken) => {
    const items = [];
    let pageToken;
    let nextSyncToken = '';
    ...
    do {
      ...
      else if (options.timeMin) params.timeMin = options.timeMin;

      const result = await calendar.events.list(params);
      items.push(...(result.data.items || []));
      pageToken = result.data.nextPageToken || undefined;
      if (!pageToken) nextSyncToken = result.data.nextSyncToken || '';
    } while (pageToken);
  ...
  if (!options.syncToken) {
    return { ...(await fetchAllPages('')), restartedAfter410: false };
  }

  try {
    return { ...(await fetchAllPages(options.syncToken)), restartedAfter410: false };
  } catch (err) {
    if (!isExpiredSyncTokenError(err)) throw err;
    return { ...(await fetchAllPages('')), restartedAfter410: true };
  }
}

function planGoogleCancellations(localRows, googleRows) {
  ...
  const deleteLocal = localRows.filter((row) => {
    const googleEventId = String(row?.google_event_id || '').trim();
    return googleEventId && cancelledGoogleEventIds.has(googleEventId);
  });
  const deletedLocalIds = new Set(deleteLocal.map((row) => String(row.id)));

  return {
    cancelledGoogleEventIds,
    deleteLocal,
    remainingLocalRows: localRows.filter((row) => !deletedLocalIds.has(String(row.id))),
    activeGoogleRows: googleRows.filter((event) => event?.status !== 'cancelled')
  };
}
