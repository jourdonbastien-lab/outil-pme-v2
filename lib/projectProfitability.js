...
function detectLineCostCategory(line = {}) {
  ...
  const text = normalizeText(`${recorded} ${label}`);
  const recordedLabor = /(^|\s)(main d.?oeuvre|atelier|pose|etude|étude|deplacement|déplacement)(\s|$)/.test(recorded);
  const labelLabor = /(main d.?oeuvre|fabrication|pose|etude|étude|atelier|deplacement|déplacement)/.test(label);
  if (recordedLabor || labelLabor) {
    if (/(pose|installation)/.test(`${recorded} ${label}`)) return 'main-d’œuvre pose';
    return 'main-d’œuvre atelier';
  }
  const candidates = [
...
function analyzeQuoteLines({ quote = {}, lines = [], adjustments = [], rules = PROFITABILITY_RULES } = {}) {
  ...
  const analyzedLines = safeLines.map((line) => {
    ...
    const storedTotalCost = optionalNumber(line.cost_total ?? line.purchase_total ?? line.purchaseTotal ?? line.total_purchase_price);
    const storedUnitCost = optionalNumber(line.cost_unit ?? line.purchase_unit_price ?? line.purchaseUnitPrice ?? line.buy_unit_price);
    const explicitCost = optionalNumber(line.cost_price ?? line.cost ?? line.prix_revient);
    const marginInput = optionalNumber(line.margin_pct ?? line.margin ?? line.marginInput);
    const coefficientInput = optionalNumber(line.coefficient ?? line.sale_coefficient ?? line.coefficientInput);
    ...
    if (Number.isFinite(storedTotalCost) && storedTotalCost >= 0) {
      cost = round2(storedTotalCost); origin = 'Prix d’achat total enregistré'; costSource = 'purchase_total';
    } else if (Number.isFinite(storedUnitCost) && storedUnitCost >= 0 && qty > 0) {
      cost = round2(qty * storedUnitCost); origin = 'Quantité × prix d’achat'; costSource = 'quantity_times_purchase_unit';
    } else if (Number.isFinite(explicitCost) && explicitCost >= 0) {
      cost = round2(explicitCost); origin = 'Prix de revient enregistré'; costSource = 'explicit_cost';
    } else if (Number.isFinite(coefficientInput) && coefficientInput > 0 && saleHT >= 0) {
      cost = round2(saleHT / coefficientInput); origin = 'Calcul depuis le coefficient du devis'; costSource = 'sale_divided_by_coefficient';
    } else if (Number.isFinite(marginInput) && marginInput > -100 && saleHT >= 0) {
      cost = round2(saleHT / (1 + marginInput / 100)); origin = 'Calcul depuis la marge du devis'; costSource = 'margin_formula';
    } else if (isLabor) {
      ...
      if (hours !== null) {
        ...
        costSource = 'hours_times_internal_rate';
      }
    }
    ...
    return {
      ...
      qty, quantity: qty, unit: String(line.unit || ''), purchaseUnitPrice: storedUnitCost,
      purchaseTotal: storedTotalCost ?? (storedUnitCost !== null && qty > 0 ? round2(storedUnitCost * qty) : null),
      saleUnitPrice: nonNegative(line.unit_price), saleTotal: saleHT, saleHT, marginInput, coefficientInput,
      detectedCost: cost, cost, margin, marginOnSale, hours, costSource,
